        ...
        with no_grad():
            ...
            while flag:
                ...
                for sample in dataloader:
                    feature = sample['x']
                    label = sample['label']
                    if self.get_prediction(feature) != label:
...
class RegressionModel(Module):
    ...
    def __init__(self):
        ...
        super().__init__()
        self.hidden_layer1 = Linear(1, 128)
    ...
    def forward(self, x):
        ...
        """
        y = x
        y = self.activation(self.hidden_layer1(y))
        y = self.activation(self.hidden_layer2(y))
...
class DigitClassificationModel(Module):
    ...
    def __init__(self):
        ...
        input_size = 28 * 28
        output_size = 10
        self.hidden_layer1 = Linear(784, 128)
        self.hidden_layer2 = Linear(128, 64)
...
class LanguageIDModel(Module):
    ...
    def __init__(self):
        ...
        "*** YOUR CODE HERE ***"
        self.length_d1 = 500
        self.init_layer = Linear(self.num_chars, self.length_d1)
        self.hidden_layer1 = Linear(self.length_d1, self.length_d1)
        self.output_layer = Linear(self.length_d1, len(self.languages))
        self.activation = relu
    ...
    def run(self, xs):
        ...
        Returns:
            A node with shape (batch_size x 5) containing predicted scores
                (also called logits)
        """
        flag = False #indicates if was first
        h = None
        for x in xs:
            if flag:
                h = self.activation(self.init_layer(x) + self.hidden_layer1(h))
            else:
                h = self.activation(self.init_layer(x))
                flag = True
    ...
    def train(self, dataset):
        ...
        for _ in range(number_of_iterations):
            for sample in dataloader:
                ...
                feature = sample['x']
                label = sample['label']
                feature = movedim(feature, 0, 1)
                loss = self.get_loss(feature, label)
                loss.backward()
                optimizer.step()
            res_acc = dataset.get_validation_accuracy()
            print(f"Step {_}: Loss = {loss.item()}, Acc = {res_acc}")
            if res_acc >= 0.83:
                break
...
def Convolve(input: tensor, weight: tensor):
    ...
    for i in range(output_height):
        for j in range(output_width):
            ...
            Output_Tensor[i, j] = (subtensor * weight).sum()
...
class DigitConvolutionalModel(Module):
    ...
    def __init__(self):
        ...
        self.hidden_layer1 = Linear(26 * 26, 128)
        self.hidden_layer2 = Linear(128, 64)
        self.output_layer = Linear(64, 10)
        self.activation = relu
