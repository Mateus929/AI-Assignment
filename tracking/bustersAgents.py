...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        best_action = None
        best_closest_ghost = float("inf")
        for action in legal:
            successor = Actions.getSuccessor(pacmanPosition, action)
            closest_ghost = float("inf")
            for pos in positions:
                closest_ghost = min(closest_ghost, self.distancer.getDistance(pos, successor))
            if closest_ghost < best_closest_ghost:
                best_closest_ghost = closest_ghost
                best_action = action
