...
def constructBayesNet(gameState: hunters.GameState):
    ...
    "*** YOUR CODE HERE ***"
    agents = [GHOST0, GHOST1, PAC]
    obs = [OBS0, OBS1]
    #populate `variables` with the Bayes Net nodes
    variables = agents + obs
    ...
    for x in range(X_RANGE):
        for y in range(Y_RANGE):
            for agent in agents:
                variableDomainsDict[agent].append((x, y))

    for dist in range(X_RANGE + Y_RANGE + MAX_NOISE - 1):
        for var in obs:
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        eliminate_variables = bayesNet.variablesSet() - set(queryVariables) - set(evidenceDict.keys())
        for variable in eliminationOrder:
            if not variable in eliminate_variables:
                continue
            factors, remained_factor = joinFactorsByVariable(factors, variable)
            if len(remained_factor.unconditionedVariables()) > 1:
                remained_factor = eliminate(remained_factor, variable)
                factors.append(remained_factor)
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        if noisyDistance is None:
            return 1 if jailPosition == ghostPosition else 0
        trueDistance = util.manhattanDistance(pacmanPosition, ghostPosition)
        return busters.getObservationProbability(float(noisyDistance), float(trueDistance))
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for oldPos in self.allPositions:
            ...
            for pos, prob in newPosDist.items():
                new_beliefs[pos] += prob * self.beliefs[oldPos]
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        for i in range(self.numParticles):
            self.particles.append(positions[i % num_positions])
